...
- (void)viewDidLoad {
    ...
    self.tableView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 50);//self.virtuelTableView.frame;
...
- (void)creatCommentListDataControllerWithFeed:(UMComFeed *)feed
{
    if (!self.dataController) {
        UMComFeedCommnetListDataController *feedCommentDataController = [[UMComFeedCommnetListDataController alloc] initWithCount:UMCom_Limit_Page_Count feedID:feed.feedID replyUserID:nil commentSortType:UMComCommentSortType_Default];
...
- (void)createTopBarItems
{
    ...
    UIImage *image = UMComSimpleImageWithImageName(@"um_more");
    ...
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            [weakSelf.feedDetailDataController likeFeedWithCompletion:^(id responseObject, NSError *error) {
                ...
                [weakSelf handleDataWhenLikeStatusChangeWithFeed:feed error:nil];
                 */
            }];
        }
    }];
...
- (void)clickOnMoreButton:(id)sender {
    ...
    NSString *title = @"";
    if ([self.feedDetailDataController checkAuthDeleteFeed]) {
...
- (void)customObj:(id)obj clickOnDeleteComment:(UMComComment *)comment
{
    __weak typeof(self) weakSelf = self;
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        [weakSelf.feedDetailDataController deletedComment:comment completion:^(id responseObject, NSError *error) {
            if (error) {
                [UMComShowToast showFetchResultTipWithError:error];
            }else{
                [UMComShowToast deleteCommentSuccess];
                [weakSelf.dataController.dataArray removeObject:comment];
                [weakSelf.tableView reloadData];
...
- (void)customObj:(id)obj clickOnSpamComment:(UMComComment *)comment
{
    __weak typeof(self) weakSelf = self;
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        [weakSelf.feedDetailDataController spamComment:comment completion:^(id responseObject, NSError *error) {
    ...
    switch (indexPath.section) {
        ...
        case 2:
        {
            ...
            if ([self.feedDetailDataController.feed.liked boolValue]) {
                ...
                [cell.likeButton setBackgroundColor:UMComColorWithColorValueString(@"#469EF8")];
                ...
                [cell.likeButton setTitleColor:UMComColorWithColorValueString(@"#469EF8") forState:UIControlStateNormal];
            }
            retCell = cell;
        }
            break;
