...
- (void)viewDidLoad
{
    ...
    if (self.isAutoStartLoadData == YES) {
        [self refreshData];
    }
...
- (void)refreshDataCompletion:(void (^)())completion
{
    ...
    [self.dataController refreshNewDataCompletion:^(NSArray *responseData, NSError *error) {
        ...
        //此处回调更新tableview,配合completion里面的tableview的代码，才能保证刷新全部数据，目前原因不明
        if ([responseData isKindOfClass:[NSArray class]] && responseData.count > 0) {
                [weakself.tableView reloadData];
        }
        
        //即使completion有[weakself.tableView reloadData]的代码，也不能完全更新全部的数据，会有一部分数据更新不全，目前原因不明
        if (completion) {
            completion();
        }
    }];
