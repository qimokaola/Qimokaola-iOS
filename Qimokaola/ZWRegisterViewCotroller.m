...
- (void)createSubViews {
   // __weak __typeof(self) weakSelf = self;
    ...
    self.phoneNumberLine = [self commonSeparatorLine];
    ...
    [_indicator mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.nextBtn.titleLabel.mas_left).with.offset(- margin);
        make.centerY.equalTo(self.nextBtn);
    }];
    
    [self.containerView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.nextBtn.mas_bottom).with.offset(largeMargin);
    }];

}

- (UIView *)commonSeparatorLine {
    UIView *separatorLine = [[UIView alloc] init];
    separatorLine.backgroundColor = defaultSeparatorLineColor;
